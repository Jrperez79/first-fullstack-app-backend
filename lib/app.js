...
app.get('/mls', async(req, res) => {
  const data = await client.query(`
      SELECT m.id, league_standing, ever_won_a_championship, c.name AS conference_name 
        FROM mls AS m 
        JOIN conferences AS c 
        ON g.conference_id = c.id
      `);
...
app.put('/mls/:id', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
      UPDATE mls
        SET name=$1, conference_id=$2, league_standing=$3, ever_won_a_championship=$4
        WHERE mls.id = $5
        RETURNING *
        `, [updatedMls.name, updatedMls.conference_id, updatedMls.league_standing, updatedMls.ever_won_a_championship, mlsId]);
...
app.post('/mls', async(req, res) => {
  ...
  const data = await client.query(`
  INSERT INTO mls(name, conference_id, league_standing, ever_won_a_championship, owner_id)
  VALUES($1, $2, $3, $4, $5)
  RETURNING *
  `, [newMlsTeam.name, newMlsTeam.conference_id, newMlsTeam.league_standing, newMlsTeam.ever_won_a_championship, fakeUser.id]);
